...
public class ChatbotController {
    ...
    public ResponseEntity<Map<String, Object>> generateSentences(@RequestBody Map<String, Object> request) {
        ...
        List<String> levels = request.get("levels") != null ? (List<String>) request.get("levels") : java.util.Arrays.asList("B1");
        @SuppressWarnings("unchecked")
        List<String> lengths = request.get("lengths") != null ? (List<String>) request.get("lengths") : java.util.Arrays.asList("medium");
        ...
        if (levels.isEmpty()) levels = java.util.Arrays.asList("B1");
        if (lengths.isEmpty()) lengths = java.util.Arrays.asList("medium");
        
        String normalizedWord = word.trim().toLowerCase();
        String cacheKey = CACHE_KEY_PREFIX + normalizedWord + ":" + String.join(",", levels) + ":" + String.join(",", lengths);
        ...
        try {
            ...
            // Tüm kombinasyonları tek bir prompt'ta belirtip, LLM'den hepsini birden üretmesini iste
            ...
            levelLengthInfo.append("Distribute the 5 sentences across these combinations. Make sentences diverse and cover different meanings if the word has multiple meanings.");
            ...
            // LLM bazen açıklama metni ekliyor, JSON array'i bul (ilk [ karakterinden başla)
                ...
                if (parsed instanceof List) {
                    ...
                    if (map.containsKey("sentences") && map.get("sentences") instanceof List) {
                        ...
                        try {
                            ...
                            throw new RuntimeException("LLM returned unexpected JSON format. Expected array or object with 'sentences' key.", ex);
                        }
                    }
                } else {
            ...
            System.out.println("Generated " + allSentences.size() + " structured sentences for word: " + normalizedWord);
    ...
    public ResponseEntity<Map<String, Object>> checkTranslation(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            // Be supportive and generous in evaluation - mark as correct if meaning is conveyed well
            String combinedMessage = "English sentence: " + englishSentence + ". User's Turkish translation: " + userTranslation + ". Evaluate this translation generously. If it conveys the meaning well, mark it as CORRECT. Only mark as INCORRECT if there are significant errors. Return ONLY a JSON object with isCorrect, correctTranslation, and feedback fields.";
        ...
        try {
            ...
            if (sentences != null && !sentences.isEmpty()) {
                for (String sentenceStr : sentences) {
                    // Sentences now only contain English text (no Turkish translation in parentheses)
    ...
    public ResponseEntity<Map<String, Object>> generateSpeakingTestQuestions(@RequestBody Map<String, String> request) {
        ...
        try {
            String message = String.format("Generate %s Speaking test questions for %s. Return ONLY JSON.", testType, part);
    ...
    public ResponseEntity<Map<String, Object>> evaluateSpeakingTest(@RequestBody Map<String, String> request) {
        ...
        try {
            String message = String.format("Evaluate this %s Speaking test response. Question: %s. Candidate's response: %s. Return ONLY JSON.", 
